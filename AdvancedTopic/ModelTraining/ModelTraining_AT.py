...
def Model_training_AT(feat_dir,model_dir):
    ...
    y1 = np.array(data1.drop(['health_score',	'Population',	'SocioEconomic_Score'], axis=1)).reshape(-1,)

    # Feature Generation 2
    X2 = data2.drop(['covid_score'], axis=1)
    y2 = np.array(data2.drop(['health_score',	'Population',	'SocioEconomic_Score'], axis=1)).reshape(-1,)

    #### RANDOM FOREST FEATURE GENERATION 1
    ...
    joblib.dump(model_config, model_dir+'Random_forest_fg1_AT.joblib')

    #### RANDOM FOREST FEATURE GENERATION 2
    #Grid search for hyperparameter tuning and cross validation using 5 folds
    param_grid = {
        'n_estimators': [10, 50, 100, 200],
        'max_depth': [None, 10, 20, 30],
    ...
    # Saving the model
    model_config = best_estimator.get_params()
    joblib.dump(model_config, model_dir+'Random_forest_fg2_AT.joblib')


    #### NEURAL NETWORKS FEATURE GENERATION 1 
    param_grid = {
    ...
    # Saving the model
    model_config = best_estimator.get_params()
    joblib.dump(model_config, model_dir+'Neural_network_fg1_AT.joblib')

    #### NEURAL NETWORKS FEATURE GENERATION 2
    param_grid = {
    'hidden_layer_sizes': [(50,), (100,), (50, 50), (100, 50)],
    'activation': ['relu', 'tanh'],
    'alpha': [0.0001, 0.001, 0.01],
    ...
    print(f'Best Model Mean Squared Error: {mse}') 
    model_config = best_estimator.get_params()
    joblib.dump(model_config, model_dir+'Neural_network_fg2_AT.joblib')


    #### SVM FEATURE GENERATION 1
    #Grid search for hyperparameter tuning and cross validation using 5 folds
    tuned_parameters = [
    ...
    print(f'Best Model Mean Squared Error: {mse1}')

    # Saving the model
    model_config = best_estimator.get_params()
    joblib.dump(model_config, model_dir+'SVM_fg1_AT.joblib')

    #### SVM FEATURE GENERATION 2
    #SVR on data2
    ...
    # Saving the model
    model_config = best_estimator.get_params()
    joblib.dump(model_config, model_dir+'SVM_fg2_AT.joblib')
